...
- **Reactive event pipeline** — `SendEvent` ヘルパーを生成し、ユーザーが [UniRx](https://github.com/neuecc/UniRx) の任意のオペレータで自由にパイプラインを構成できる
...
- [UniRx](https://github.com/neuecc/UniRx) (UPM)
...
{
  "dependencies": {
    "com.cortis.core": "https://github.com/gatari/cortis.git?path=Packages/com.cortis.core",
...
public sealed class MyMessageGateway : IMessageGateway, IDisposable
{
    ...
    public IObservable<Any> Messages => _messages;
...
(Flutter, Native, etc.)                                                  ↑
                                                                [ProtoHandler] で自動生成
                                                User Observable → [UniRx operators] → SendEvent()
```
...
**イベントフロー (Unity → 外部):**
`User Observable` → `[UniRx operators]` → `SendEvent()` → `Any.Pack()` → `Gateway.Send()`
...
User Observable → [UniRx operators] → SendEvent() → wrap(inner) → Any.Pack() → Gateway.Send()
```
